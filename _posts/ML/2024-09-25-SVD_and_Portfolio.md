...
Combining this explained variance in singluar values, $\( V^{T} \)$, can be used to explain portfolio. In here we are going to suppose that we have portfolio with N assets, and each asset have equal weight $\frac{1}{N}$. If we use only $ \sigma_1 $ for dimensionality reduction, portfolio with weights $ \vec{u}_1 \times original_weight $ =  $ \( vec{u}_1, vec{u}_2, ... ,vec{u}_{10} \) \times \( \frac{1}{10}, ... , \frac{1}{10} \) $ explains $ \frac{\sigma_1}{\sigma_1 + \sigma_2} $ variance of original portfolio.
